...
def registration_page() -> rx.Component:
    ...
    register_form = rc.form(
        rc.input(placeholder="username", id="username"),
        rc.password(placeholder="password", id="password"),
        rc.password(placeholder="confirm", id="confirm_password"),
        rc.button("Register", type_="submit"),
        width="80vw",
    ...
    return rx.fragment(
        rx.cond(
            ...
            rc.vstack(
                rc.text("Registration successful!"),
                rc.spinner(),
            ),
            rc.vstack(
