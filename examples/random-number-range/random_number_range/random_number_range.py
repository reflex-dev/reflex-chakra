...
other_links = {
    "Chain Events": lambda State: rc.link("Background Task Version", href="/background", on_click=State.set_running(False)),
    "Background Task": lambda State: rc.link("Chain Event Version", href="/chain", on_click=State.set_running(False)),
}
...
def random_numbers_in_range(State, mode: str) -> rx.Component:
    return rc.center(
        rc.vstack(
            rc.heading(f"Random Numbers in Range"),
            rc.heading(f"{mode} version", font_size="1.5em"),
            other_links[mode](State),
            rc.hstack(
                rc.text("Min: ", State.rrange[0], padding_right="3em"),
                rc.button("Balance", on_click=State.balance),
                rc.text("Max: ", State.rrange[1], padding_left="3em"),
            ),
            rc.range_slider(value=State.rrange, on_change=State.set_rrange, min_=-100, max_=100),
            rc.hstack(
                rc.text("Last 10 values: ", State.last_values),
                rx.cond(State.loading, rc.spinner()),
            ),
            rc.hstack(
                rc.text("Total: ", State.total),
                rc.button("Clear", on_click=lambda: State.set_total(0)),
            ),
            rc.hstack(
                rc.vstack(
                    rc.text("Run", font_size="0.7em"),
                    rc.switch(is_checked=State.running, on_change=State.set_running),
                ),
                rc.vstack(
                    rc.text("Delay (sec)", font_size="0.7em"),
                    rc.select(*[rc.option(x) for x in range(1, 5)], value=State.delay.to(str), on_change=State.set_delay),
                    padding_right="3em",
                ),
                rc.button("Single Step", on_click=State.single_step),
